...
const culprits = board => {
  //construct array of squares coords repeat more than 50% length in row or column
  const moreThanHalf = board.map((row, y) => {
  ...
  let culpritsCoords = [].concat.apply(
    [],
    moreThanHalf.map(r => r.filter(v => v != null))
  );
  //construct array of consecutive squares of 3
...
const numSquaresFilled = (board, correctOnly = false) => {
  ...
  if (correctOnly) {
    ...
    squareFilledCorrect += columns(board).reduce((acc, cols) => {
